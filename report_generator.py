...
def fmt_val(val):
    """Format a number with no decimals if it's a whole number, else 2 decimal places."""
    v = safe_float(val)
...
def _h_bar_chart(bkfc_vals, opp_vals, labels, title, width=10.5, height=4.8, dpi=160):
    ...
    for bar in b1:
        ...
        ax.text(w + max(bkfc_vals + opp_vals) * 0.01, bar.get_y() + bar.get_height()/2,
                fmt_val(w), va="center", ha="left", fontsize=7.5, fontweight="bold",
                color="#" + COLORS["BLACK"])
    for bar in b2:
        w = bar.get_width()
        ax.text(w + max(bkfc_vals + opp_vals) * 0.01, bar.get_y() + bar.get_height()/2,
                fmt_val(w), va="center", ha="left", fontsize=7.5,
...
def add_summary_slide(prs, data):
    ...
    for ri, (label, col) in enumerate(summary_metrics, start=1):
        ...
        _set_cell(tbl.cell(ri, 0), label,
                  size=11, bg_hex=bg)
        _set_cell(tbl.cell(ri, 1), fmt_val(bkfc_val),
                  size=11, bold=True, bg_hex=bg,
                  align=PP_ALIGN.CENTER)
        _set_cell(tbl.cell(ri, 2), fmt_val(opp_val),
                  size=11, bold=True, bg_hex=bg,
                  align=PP_ALIGN.CENTER)
...
def add_defensive_slide(prs, data):
    ...
    def _def_chart(bkfc_v, comp_v, comp_label, title, comp_color="DARK_GRAY"):
        ...
        b2 = ax.bar(x + bar_w/2, comp_v, bar_w,
                    color="#" + COLORS[comp_color], label=comp_label,
                    edgecolor="none")
        ax.set_xticks(x)
        ax.set_xticklabels(labels, fontsize=6.5, rotation=30, ha="right",
                           color="#" + COLORS["BLACK"])
